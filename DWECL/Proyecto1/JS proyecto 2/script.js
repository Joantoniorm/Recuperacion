...
for (const countryName of allCountries) {
    string += `<tr data-name="${countryName.name}"><td>${countryName.name}</td></tr>`
}
...
if (option !== "") {
    ...
    let tFiltered = document.querySelector("tbody");
    tFiltered.innerHTML = "";
    for (const continent of companies) {
        if (continent.continent === option) {
            for (const country of continent.countries) {
                tFiltered.innerHTML += `<tr data-name="${country.name}"><td>${country.name}</td></tr>`;
            }
        }
    }
    showCompaniesNum();
